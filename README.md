...
Charm은 다음 AI providers를 지원합니다. 기본 UX는 REPL 안에서 `/provider connect <id>`로 연결하고 `~/.charm/auth.json`에 저장하는 방식입니다. 환경 변수도 계속 지원합니다.
...
- **컴파일 워닝**: CI는 `cargo clippy --all-targets --all-features -- -D warnings`로 warning-free 상태를 강제합니다.
