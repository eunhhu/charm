...
### OpenAI
- **Charm auth store**: `/provider connect openai`로 `~/.charm/auth.json`에 저장
...
### OpenAI Codex
- **Charm auth store**: `/provider connect openai-codex`로 `~/.charm/auth.json`에 저장
- **인증 파일**: `~/.codex/auth.json` (Codex CLI와 공유)
- **환경 변수**: `CODEX_HOME`으로 경로 지정 가능
- **기존 연결 방법**: `codex login`
...
### Anthropic (Claude)
- **Charm auth store**: `/provider connect anthropic`로 `~/.charm/auth.json`에 저장
...
### Google (Gemini)
- **Charm auth store**: `/provider connect google`로 `~/.charm/auth.json`에 저장
...
### OpenRouter
- **Charm auth store**: `/provider connect openrouter`로 `~/.charm/auth.json`에 저장
...
1. 모델 이름의 provider 접두사 (`provider/model`)
2. 모델 이름 기반 자동 감지
3. 환경 기본값 (OpenRouter)
...
- 미연결 provider를 선택하거나 `/provider connect <provider>`를 실행하면 REPL 안에서 auth wizard가 열리고 `~/.charm/auth.json`에 저장됩니다.
...
### "API key must be set"
TUI에서 `/provider connect <provider>`를 실행해 연결하거나, 해당 provider의 환경 변수를 설정하세요.
